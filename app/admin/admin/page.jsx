...
export default function AdminPage() {
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        ...
        toast.error("Админ маълумотларни юклашда хатолик юз берди.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Админ маълумотлари янгиланди");
      setIsEditing(false);
    } catch (error) {
    ...
    <div className="container mx-auto py-10">
      <Card className="max-w-2xl mx-auto">
        <CardHeader className="flex justify-between items-center">
          <CardTitle className="text-3xl font-bold">Админ</CardTitle>
        ...
        <CardContent>
          {isEditing ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="password">Янги Парол</Label>
                <Input id="password" name="password" type="password" value={admin.password} onChange={handleInputChange} placeholder="Янги парол киритинг" disabled={isSubmitting} />
              </div>
              <div className="flex gap-4">
                <Button type="submit" className="w-full hover:bg-primary" disabled={isSubmitting}>
                  {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Сақлаш"}
                </Button>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                <span className="text-lg font-semibold text-gray-700">Исм</span>
