...
export default function BannerEvent({ params }) {
  const { banner: bannerId } = use(params); // Assuming route is /admin/banners/[id]
  ...
  const uploadFile = async (fileToUpload) => {
    ...
    formData.append("image", fileToUpload.file, "banner.webp");
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      let result;
      if (isAddMode) {
        result = await postData(updatedValues, "/api/banners", "banner");
      } else {
      ...
      if (!result.error && result) {
        if (isAddMode) {
          toast.success("Баннер мувофаққиятли қўшилди");
          form.reset();
          setFiles([]);
        } else {
          toast.info("Баннер мувофаққиятли янгиланди");
        }
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto py-10">
        <h1 className="text-2xl font-bold mb-6">
          {isAddMode
            ? "Янги баннер қўшиш"
            : `Баннерни таҳрирлаш (ID: ${bannerId || "номаълум"})`}
        </h1>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Баннер URL ни киритинг" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
