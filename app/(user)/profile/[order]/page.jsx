...
export default async function OrderPage({ params }) {
  ...
  if (!orderData) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <Link href="/admin/orders" className="mt-4 text-blue-500 flex items-center">
  ...
  const { order_type, price, bonus, user_id, phone, name, organization, inn, comment, order_items } = orderData;

  return (
    <div className="container mx-auto py-10">
      {/* Back Button */}
      <Link href="/admin/orders" className="mb-6 flex items-center text-blue-500">
        <ArrowLeft className="mr-2 h-4 w-4" /> Орқага қайтиш
      </Link>
      ...
      <Card className="mb-8">
        <CardHeader>
          <CardTitle className="text-3xl font-bold">
            Буюртма #{orderId} - {order_type === "individual" ? "Жисмоний шахс" : "Юридик шахс"}
          </CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <p className="text-sm text-gray-500">{order_type === "individual" ? "Исм" : "Ташкилот"}</p>
            <p className="text-lg font-semibold">{order_type === "individual" ? name : organization || "-"}</p>
          </div>
          {order_type === "legal" && (
            <div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold">{price.toLocaleString()} сўм</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Бонус</p>
            <p className="text-lg font-semibold">{bonus.toLocaleString()} сўм</p>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold">Буюртма маҳсулотлари</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              ...
              {order_items.map((item, idx) => {
                ...
                return (
                  <TableRow key={idx}>
                    ...
                    <TableCell>{(item?.price * item?.order_quantity).toLocaleString()} сўм</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
